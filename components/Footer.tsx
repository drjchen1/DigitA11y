...
const Footer: React.FC<FooterProps> = ({ isReadingMode }) => {
  ...
  const julianVersion = typeof __JULIAN_VERSION__ !== 'undefined' ? __JULIAN_VERSION__ : '2461284.02';
  const gregorianDate = formatJulianToGregorian(julianVersion);
  const tooltipText = gregorianDate ? `Gregorian Date: ${gregorianDate}` : 'Julian Date';
  ...
  return (
    <footer className="w-full py-6 border-t border-zinc-200/60 mt-auto flex items-center justify-center select-none">
      <div className="text-[11px] font-mono text-zinc-400 tracking-wider flex items-center gap-2">
        ...
        <span
          className="text-zinc-500 font-mono cursor-help transition-colors hover:text-zinc-700"
          title={tooltipText}
        >
          v{julianVersion}
        </span>
